 ...
 * @author Dies
 */
public class CtrlConfColor extends ctrlConexion implements IAccionesDB {

    ctrlConexion ctrlConexion;

    public CtrlConfColor() throws SQLException {
    ...
    public boolean Agregar(Object _O) {
       try {
            ConfColor objTemp = (ConfColor) _O;
            PreparedStatement Agregar;
            Agregar = ctrlConexion.getCon().prepareStatement("Insert into confColor (strvalor,strdescripcion,idconftipocolor) values (?,?,?)");
            Agregar.setString(1, objTemp.getStrValor());
            Agregar.setString(2, objTemp.getStrDescripcion());
            Agregar.setInt(3, objTemp.getIdConfTipoColor());
            Agregar.execute();
            Agregar.close();
            return true;
    ...
    public boolean Editar(Object _O) {
       try {
            ...
            String temp = "Update confMarca set strvalor ='" + objTemp.getStrValor() + "',strDescripcion ='" + objTemp.getStrDescripcion()
                    + "',idconfestadoglobal=" + objTemp.getIdConfTipoColor() + " where ID = ?;";

            System.out.println(temp);
            PreparedStatement Modificar;
            Modificar = ctrlConexion.getCon().prepareStatement("Update confColor set strvalor ='" + objTemp.getStrValor() + "',strDescripcion ='" + objTemp.getStrDescripcion()
                    + "',idconftipocolor=" + objTemp.getIdConfTipoColor() + " where ID = ?");
    ...
    public boolean Eliminar(Object O) {
        ConfColor objTemp = (ConfColor) O;
        try {
            PreparedStatement Eliminar;
            Eliminar = ctrlConexion.getCon().prepareStatement("Delete from confColor where id=?;");
    ...
    public ArrayList<Object> ConsultaCompleja() {
         ArrayList<ConfColor> consulta = new ArrayList();
        PreparedStatement consultaCompleja;
        ResultSet resultado;

        try {
            consultaCompleja = ctrlConexion.getCon().prepareStatement("Select * from confColor;");
            resultado = consultaCompleja.executeQuery();
            while (resultado.next()) {
                ConfColor temp = new ConfColor();
    ...
    public ArrayList<Object> ConsultaCompleja(Object _O) {
        ...
        try {
            consultaCompleja = ctrlConexion.getCon().prepareStatement("Select * from confColor where idconftipoColor = " + idEstado + ";");
            resultado = consultaCompleja.executeQuery();
            while (resultado.next()) {
                ConfColor temp = new ConfColor();
                temp.setId(Integer.parseInt(resultado.getString(1)));
    ...
    public ArrayList<Object> ConsultaEspecializada(Object _O) {
        try {
            ...
            ResultSet resultado;
            ConfColor objTemp = (ConfColor) _O;
            String consultaBusqueda = "SELECT * FROM confColor";

            if (objTemp.getIdConfTipoColor() > 0) {
                consultaBusqueda += " where idConfTipoColor = " + objTemp.getIdConfTipoColor();
            }
    ...
    public ArrayList<Object> ConsultaParaManager(Object _O) {
        ...
        try {
            consultaCompleja = ctrlConexion.getCon().prepareStatement("Select * from confColor where id = " + idMarca + ";");
            resultado = consultaCompleja.executeQuery();
            while (resultado.next()) {
                ConfColor temp = new ConfColor();
                temp.setId(Integer.parseInt(resultado.getString(1)));
                temp.setStrValor(resultado.getString(2));
                temp.setStrDescripcion(resultado.getString(3));
                temp.setIdConfTipoColor(resultado.getInt(4));

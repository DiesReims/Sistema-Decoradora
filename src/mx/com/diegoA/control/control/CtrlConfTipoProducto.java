 ...
 * @author Dies
 */
public class CtrlConfTipoProducto extends ctrlConexion implements IAccionesDB {

    ctrlConexion ctrlConexion;

    public CtrlConfTipoProducto() throws SQLException {
    ...
    public ArrayList<Object> ConsultaCompleja() {
        ArrayList<ConfTipoProducto> consulta = new ArrayList();
        PreparedStatement consultaCompleja;
        ResultSet resultado;

        try {
            consultaCompleja = ctrlConexion.getCon().prepareStatement("Select * from ConfTipoproducto");
            resultado = consultaCompleja.executeQuery();
            while (resultado.next()) {
                ConfTipoProducto temp = new ConfTipoProducto();
            ...
            Logger.getLogger(CtrlConfTipoProducto.class.getName()).log(Level.SEVERE, null, ex);
            return null;
        }
